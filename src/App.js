...
function App() {
  ...
  const distinctYears = [...new Set(expensesdata.map((data) => { return data.date.getFullYear().toString() }))]
  // console.log(distinctYears)
  const dataHandler = (formdata) => {
    setexpensesdata((oldstate) => {console.log(formdata); return [...oldstate, formdata] })
  }
  return (
    <div>
      <NewExpense dataHandler={dataHandler}></NewExpense>
      <Expenses data={expensesdata} distinctYears={distinctYears}></Expenses>
